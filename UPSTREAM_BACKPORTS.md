...
| 8 | `5ca0fa73` + `e62beff1` + `0637d1ad` + `fd81fd18` | bakery software raster + baking off-thread + meshing sans limite (**le plus gros**) | 🔄 en cours (réimpl par étapes) | Réimplémentation vs API 1.21.1 (l'upstream cible MC 26.1). **Stage 1** ✅ PR #7 : `SoftwareRasterizer` (CPU, inerte). **Stage 2** 🔄 : `SoftwareModelTextureBakery` (extraction quads 1.21.1 réutilisée de `ModelTextureBakery`, projection GL pour matcher le depth de `computeDepth`) + `ModelFactory` câblé dessus, même thread (render), même layout de sortie transcodé → aval inchangé. **Stage 3** ⏳ : off-thread (`e62beff1`) + retrait limites (`0637d1ad`,`fd81fd18`, relâche l'item 3). Anciens fichiers bakery GL gardés (inutilisés ; `BudgetBufferRenderer` fournit `VERTEX_FORMAT_SIZE`) |
