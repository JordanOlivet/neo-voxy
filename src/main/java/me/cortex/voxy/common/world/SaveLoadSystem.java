...
public class SaveLoadSystem {
    ...
    private record SerializationCache(long[] blockStateCache, short[] compressedCache, long[] lutCache,
            Long2ShortOpenHashMap lutMapCache) {
        public SerializationCache() {
            this(new long[WorldSection.SECTION_VOLUME], new short[WorldSection.SECTION_VOLUME],
                    new long[WorldSection.SECTION_VOLUME], new Long2ShortOpenHashMap(512));
    ...
    public static MemoryBuffer serialize(WorldSection section) {
        ...
        UnsafeUtil.memPutLong(ptr, metadata);
        ptr += 8;

        hash ^= metadata;
        hash *= 1242629872171L;

        UnsafeUtil.memPutInt(ptr, lutIndex);
        ptr += 4;
        for (int i = 0; i < lutIndex; i++) {
            long id = lutValues[i];
            UnsafeUtil.memPutLong(ptr, id);
            ptr += 8;
            hash *= 1230987149811L;
            hash += 12831;
            hash ^= id;
    ...
    public static boolean deserialize(WorldSection section, MemoryBuffer data) {
        ...
        long key = UnsafeUtil.memGetLong(ptr);
        ptr += 8;
        if (VERIFY_MEMORY_ACCESS && data.size <= (ptr - data.address))
            throw new IllegalStateException("Memory access OOB");

        long metadata = UnsafeUtil.memGetLong(ptr);
        ptr += 8;
        if (VERIFY_MEMORY_ACCESS && data.size <= (ptr - data.address))
            throw new IllegalStateException("Memory access OOB");
        section.nonEmptyChildren = (byte) (metadata & 0xFF);

        int lutLen = UnsafeUtil.memGetInt(ptr);
        ptr += 4;
        if (VERIFY_MEMORY_ACCESS && data.size <= (ptr - data.address))
            throw new IllegalStateException("Memory access OOB");
        if (lutLen > 32 * 32 * 32) {
        ...
        for (int i = 0; i < lutLen; i++) {
            lut[i] = UnsafeUtil.memGetLong(ptr);
            ptr += 8;
            if (VERIFY_MEMORY_ACCESS && data.size <= (ptr - data.address))
                throw new IllegalStateException("Memory access OOB");
            if (VERIFY_HASH_ON_LOAD) {
        ...
        if (section.key != key) {
            // throw new IllegalStateException("Decompressed section not the same as
            // requested. got: " + key + " expected: " + section.key);
        ...
        for (int i = 0; i < WorldSection.SECTION_VOLUME; i++) {
            long state = lut[Short.toUnsignedInt(UnsafeUtil.memGetShort(ptr))];
            ptr += 2;
            if (VERIFY_MEMORY_ACCESS && data.size <= (ptr - data.address))
                throw new IllegalStateException("Memory access OOB");
        ...
        if (VERIFY_HASH_ON_LOAD) {
            ...
            long expectedHash = UnsafeUtil.memGetLong(ptr);
            ptr += 8;
            if (VERIFY_MEMORY_ACCESS && data.size < (ptr - data.address))
                throw new IllegalStateException("Memory access OOB");
            if (expectedHash != hash) {
                // throw new IllegalStateException("Hash mismatch got: " + hash + " expected: "
                // + expectedHash);
