...
public class SaveLoadSystem2 {
    ...
    public static int lin2z(int i) {// y,z,x
        ...
        int y = (i >> 10) & 0x1F;
        int z = (i >> 5) & 0x1F;
        return Integer.expand(x, 0b1001001001001) | Integer.expand(y, 0b10010010010010)
                | Integer.expand(z, 0b100100100100100);
    ...
    private static final ThreadLocal<DeserialCache> DESERIALIZE_CACHE = ThreadLocal
            .withInitial(() -> new DeserialCache());
    ...
    public static MemoryBuffer serialize(WorldSection section) {
        ...
        // where block and biome are pelleted (0 block id (air) is implicitly in the
        // pallet )
        // if all entries in a specific array are the same, just emit that single value
        // do bitpacking on the resulting arrays for pallets/when packing the palleted
        // arrays
        // if doing bitpacking + pallet is larger than just emitting raw entries, do
        // that

        // Header includes position (long), (maybe time?), version storage type/version,
        // child existence, air block count?
        ...
        UnsafeUtil.memPutLong(ptr, section.key);
        ptr += 8;
        UnsafeUtil.memPutLong(ptr, hash);
        ptr += 8;
        ...
        UnsafeUtil.memPutInt(ptr, meta);
        ptr += 4;

        // Encode lighting
        /*
         * //Micro storage optimization, not done cause makes decode very slightly
         * slower and more pain
         * if (allSameBlockLight && allSameSkyLight) {
         * UnsafeUtil.memPutByte(ptr, (byte)
         * ((blockLight[0]&0xF)|((skyLight[0]&0xF)<<4))); ptr += 1;
         * } else
         */
        {
            if (allSameBlockLight) {
                UnsafeUtil.memPutByte(ptr, (byte) (blockLight[0] & 0xF));
                ptr += 1;
            } else {
                UnsafeUtil.memcpy(blockLight, ptr);
                ptr += blockLight.length;
            }
            if (allSameSkyLight) {
                UnsafeUtil.memPutByte(ptr, (byte) (skyLight[0] & 0xF));
                ptr += 1;
            } else {
        ...
        // Compact encoding of block and biome mappinigs
        {
            ...
            {// Block
                ...
                for (int i = 0; i < blockMapping.size(); i++) {
                    ...
                    if (rem < SIZE) {
                        UnsafeUtil.memPutInt(ptr, batch);
                        ptr += 4;
                        batch = b >> rem;
                        rem = 32 - (SIZE - rem);
                    } else {
        ...
        // TODO: see if tight bitpacking is better or if bitpacking with pow2 pack size
        // is better

        {// Store blocks
            final int SIZE = Mth
                    .smallestEncompassingPowerOfTwo(Mth.log2(Mth.smallestEncompassingPowerOfTwo(blockMapping.size())));
        ...
        {// Store biome
            if (biomeMapping.size() == 1) {
                ...
                final int SIZE = Mth.smallestEncompassingPowerOfTwo(
                        Mth.log2(Mth.smallestEncompassingPowerOfTwo(biomeMapping.size())));
    ...
    public static boolean deserialize(WorldSection section, MemoryBuffer data) {
        ...
        long pos = UnsafeUtil.memGetLong(ptr);
        ptr += 8;
        if (section.key != pos) {
            Logger.error("Section pos not the same as requested, got " + pos + " expected " + section.key);
            return false;
        }
        long chash = UnsafeUtil.memGetLong(ptr);
        ptr += 8;
        int meta = UnsafeUtil.memGetInt(ptr);
        ptr += 4;
        ...
        if (allSameBlockLight) {
            ...
            blockLight = Byte.toUnsignedLong(UnsafeUtil.memGetByte(ptr)) << 4;
            ptr += 1;
        } else {
            blockLight = ptr;
            ptr += 32 * 32 * 32 / 2;
        ...
        if (allSameSkyLight) {
            skyLight = UnsafeUtil.memGetByte(ptr);
            ptr += 1;
        } else {
            skyLight = ptr;
            ptr += 32 * 32 * 32 / 2;
        ...
        {// Deserialize the block and biome mappings
            int rem = 32;
            int batch = UnsafeUtil.memGetInt(ptr);
            ptr += 4;
            {// Block
                int SIZE = 20;// 20 bits per entry
                int msk = (1 << SIZE) - 1;
                for (int i = 0; i < blockMapSize; i++) {
                    ...
                    rem -= SIZE;
                    if (rem < 0) {
                        batch = UnsafeUtil.memGetInt(ptr);
                        ptr += 4;
        ...
        {// Block
            final int SIZE = Mth
                    .smallestEncompassingPowerOfTwo(Mth.log2(Mth.smallestEncompassingPowerOfTwo(blockMapSize)));
            int rem = 32;
            int batch = UnsafeUtil.memGetInt(ptr);
            ptr += 4;
            int msk = (1 << SIZE) - 1;
        ...
        {// Biome
            if (biomeMapSize == 1) {
                ...
                final int SIZE = Mth
                        .smallestEncompassingPowerOfTwo(Mth.log2(Mth.smallestEncompassingPowerOfTwo(biomeMapSize)));
                int rem = 32;
                int batch = UnsafeUtil.memGetInt(ptr);
                ptr += 4;
                int msk = (1 << SIZE) - 1;
        ...
        for (int i = 0; i < 32 * 32 * 32; i++) {
            byte light = 0;
            {
                if (allSameBlockLight) {
                    ...
                    light |= (byte) (((Byte
                            .toUnsignedInt(UnsafeUtil.memGetByte(blockLight + (i >> 1))) >> ((i & 1) * 4)) & 0xF) << 4);
                }
                if (allSameSkyLight) {
                    light |= (byte) (skyLight & 0xF);
                } else {
                    // Todo clean and optimize this (it can be optimized alot)
                    light |= (byte) ((Byte.toUnsignedInt(UnsafeUtil.memGetByte(skyLight + (i >> 1))) >> ((i & 1) * 4))
                            & 0xF);
                }
            }
