...
public class SaveLoadSystem3 {
    ...
    private record SerializationCache(Long2ShortOpenHashMap lutMapCache, MemoryBuffer memoryBuffer) {
        public SerializationCache() {
            this(new Long2ShortOpenHashMap(1024), ThreadLocalMemoryBuffer
                    .create(WorldSection.SECTION_VOLUME * 2 + WorldSection.SECTION_VOLUME * 8 + 1024));
    ...
    public static MemoryBuffer serialize(WorldSection section) {
        ...
        for (long block : data) {
            ...
            if (mapping == -1) {
                ...
                UnsafeUtil.memPutLong(ptr, block);
                ptr += 8;
            }
            UnsafeUtil.memPutShort(blockPtr, mapping);
            blockPtr += 2;
        }
        if (LUT.size() >= 1 << 16) {
        ...
        // TODO: note! can actually have the first (last?) byte of metadata be the
        // storage version!
        long metadata = 0;
    ...
    public static boolean deserialize(WorldSection section, MemoryBuffer data) {
        ...
        final long metadata = UnsafeUtil.memGetLong(ptr);
        ptr += 8;
        ...
        if (section.lvl == 0) {
            ...
            for (int i = 0; i < WorldSection.SECTION_VOLUME; i++) {
                final short lutId = UnsafeUtil.memGetShort(ptr);
                ptr += 2;
                final long blockId = UnsafeUtil.memGetLong(lutBasePtr + Short.toUnsignedLong(lutId) * 8L);
            ...
            final var blockData = section.data;
            for (int i = 0; i < WorldSection.SECTION_VOLUME; i++) {
                blockData[i] = UnsafeUtil
                        .memGetLong(lutBasePtr + Short.toUnsignedLong(UnsafeUtil.memGetShort(ptr)) * 8L);
                ptr += 2;
            }
        }
