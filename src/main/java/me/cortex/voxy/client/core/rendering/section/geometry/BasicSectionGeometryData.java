...
public class BasicSectionGeometryData implements IGeometryData {
    ...
    public static GlBuffer allocateGeometryBuffer(long geometryCapacity) {
        ...
        if (Capabilities.INSTANCE.canQueryGpuMemory) {
            msg += " driver states " + (Capabilities.INSTANCE.getFreeDedicatedGpuMemory() / (1024 * 1024))
                    + "MB of free memory";
        }
        Logger.info(msg);
        Logger.info(
                "if your game crashes/exits here without any other log message, try manually decreasing the geometry capacity");
        ...
        if (!(Capabilities.INSTANCE.isNvidia)) {// && ThreadUtils.isWindows
            ...
            // TODO: FIXME: TEST, see if the issue is that we are trying to zero the entire
            // buffer, try only zeroing increments
    ...
    public void ensureAccessable(int maxElementAccess) {
        ...
        // If we are a sparse buffer, ensure the memory upto the requested size is
        // allocated
    ...
    public void free() {
        ...
        glFinish();
        if (Capabilities.INSTANCE.canQueryGpuMemory) {
            long releaseSize = (long) (this.geometryBuffer.size() * 0.75);// if gpu memory usage drops by 75% of the
                                                                          // expected value assume we freed it
            ...
            if (Capabilities.INSTANCE.getFreeDedicatedGpuMemory() - gpuMemory <= releaseSize) {
                ...
                if (Capabilities.INSTANCE.getFreeDedicatedGpuMemory() - gpuMemory <= releaseSize) {
                    Logger.warn(
                            "Failed to wait for gpu memory to be freed, this could indicate an issue with the driver");
                }
            }
        }
    }
