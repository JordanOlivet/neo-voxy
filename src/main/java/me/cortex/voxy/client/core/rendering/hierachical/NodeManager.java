...
public class NodeManager {
    ...
    // Assumptions:
    // all nodes have children (i.e. all nodes have at least one child existence bit
    // set at all times)
    // leaf nodes always contain geometry (empty geometry counts as geometry (it
    // just doesnt take any memory to store))
    ...
    // There is a very funny issue that has kinda, just resolved itself
    // accidentally,
    ...
    // the child request,
    // however, since there are no children in it, it sticks around, since there
    // isnt anything to update it and invoke
    // the finishRequest on it
    // if the top level node ends up being updated with a child update, it should
    // automatically solve itself
    // as the new children are added to the already inprogress request!!!!

    // Alot of the rules for this datastructure have changed as it has evolved and
    // finished
    ...
    // Single requests are basically _only_ generated by the insertion of top level
    // nodes
    private final ExpandingObjectAllocationList<SingleNodeRequest> singleRequests = new ExpandingObjectAllocationList<>(
            SingleNodeRequest[]::new, NodeStore.REQUEST_ID_MSK);
    private final ExpandingObjectAllocationList<NodeChildRequest> childRequests = new ExpandingObjectAllocationList<>(
            NodeChildRequest[]::new, NodeStore.REQUEST_ID_MSK);
    ...
    // Rate limiting for existence mask warning to reduce log spam
    private long lastExistenceMaskWarningTime = 0;
    private int suppressedExistenceMaskWarnings = 0;
    ...
    private static void assertPosValid(long pos) {
        ...
        if (WorldEngine.getLevel(p2) != 0 || WorldEngine.getX(p2) != x || WorldEngine.getY(p2) != y
                || WorldEngine.getZ(p2) != z) {
            throw new IllegalStateException("Position not valid at all levels: " + pos + "-"
                    + WorldEngine.pprintPos(pos) + ":" + WorldEngine.pprintPos(p2));
        }
    }
    ...
    public void insertTopLevelNode(long pos) {
        ...
        if (this.activeSectionMap.containsKey(pos)) {
            Logger.error("Tried inserting top level pos " + WorldEngine.pprintPos(pos)
                    + " but it was in active map, discarding!");
            return;
        }
    ...
    public void removeTopLevelNode(long pos) {
        ...
        if (nodeId == -1) {
            throw new IllegalStateException("Tried removing top level pos " + WorldEngine.pprintPos(pos)
                    + " but it was not in active map, discarding!");
        }
        if ((nodeId & NODE_TYPE_MSK) != NODE_TYPE_REQUEST) {
            int id = nodeId & NODE_ID_MSK;
            if (!this.topLevelNodeIds.remove(id)) {
                throw new IllegalStateException(
                        "Node id was not in top level node ids: " + nodeId + " pos: " + WorldEngine.pprintPos(pos));
            }
    ...
    public void processGeometryResult(BuiltSection sectionResult) {
        ...
        int nodeId = this.activeSectionMap.get(pos);
        if (nodeId == -1) {
            // Logger.warn("Got geometry update for pos " + WorldEngine.pprintPos(pos) + "
            // but it was not in active map, discarding!");
            sectionResult.free();
            return;
        }
        ...
        if ((nodeId & NODE_TYPE_MSK) == NODE_TYPE_REQUEST) {
            ...
            if ((nodeId & REQUEST_TYPE_MSK) == REQUEST_TYPE_SINGLE) {
                ...
                // sectionResult has a cheeky childExistence field that we can use to set the
                // request too, this is just
                // because processChildChange is only ever invoked when child existence changes,
                // so we still need to
    ...
    public void processChildChange(long pos, byte childExistence) {
        int nodeId = this.activeSectionMap.get(pos);
        if (nodeId == -1) {
            Logger.warn("Got child change for pos " + WorldEngine.pprintPos(pos)
                    + " but it was not in active map, ignoring!");
            return;
        }
        ...
        if ((nodeId & NODE_TYPE_MSK) == NODE_TYPE_REQUEST) {
            ...
            if (this.nodeData.isNodeRequestInFlight(nodeId & NODE_ID_MSK)) {
                // Logger.error("UNFINISHED OPERATION TODO: FIXME: painful operation, needs to
                // account for both adding and removing, need to do the same with inner node,
                // but also create requests, or cleanup children");
                int requestId = this.nodeData.getNodeRequest(nodeId);
                var request = this.childRequests.get(requestId);// TODO: do not assume request is childRequest (it will
                                                                // probably always be)
                if (request.getPosition() != pos)
                    throw new IllegalStateException("Request is not at pos, got "
                            + WorldEngine.pprintPos(request.getPosition()) + " expected " + WorldEngine.pprintPos(pos));
                {// Update the request
                    ...
                    {// Remove children and free/release associated meshes
                        ...
                        for (int i = 0; i < 8; i++) {
                            ...
                            if (this.activeSectionMap.remove(cPos) == -1) {// TODO: verify the removed section is a
                                                                           // request type of child and the request id
                                                                           // matches this
                                throw new IllegalStateException(
                                        "Child pos was in a request but not in active section map");
                            }
                    ...
                    {// Add new children to the request
                        ...
                        for (int i = 0; i < 8; i++) {
                            ...
                            long cPos = makeChildPos(pos, i);
                            if (this.activeSectionMap.put(cPos,
                                    requestId | NODE_TYPE_REQUEST | REQUEST_TYPE_CHILD) != -1) {
                                throw new IllegalStateException(
                                        "Child pos was already in active section tracker but was part of a request");
                            }
    ...
    private void updateChildSectionsInner(long pos, int nodeId, byte childExistence) {
        ...
        // This works in 2 parts, adding and removing, adding is (surprisingly) much
        ...
        if (add != 0) {// We have nodes to add
            ...
            var request = this.childRequests.get(requestId);// TODO: do not assume request is childRequest (it will
                                                            // probably always be)
            if (request.getPosition() != pos)
                throw new IllegalStateException("Request is not at pos: got " + WorldEngine.pprintPos(pos)
                        + " expected: " + WorldEngine.pprintPos(request.getPosition()));
        ...
        // this needs to be before the removal since that may invoke requestFinish,
        // which expects updated node masks
        // TODO: verify this
        ...
        if (rem != 0) {
            ...
            if (this.nodeData.isNodeRequestInFlight(nodeId)) {
                ...
                if (reqRem != 0) {
                    ...
                    for (int i = 0; i < 8; i++) {
                        ...
                        if (cnid == -1 || (cnid & NODE_TYPE_MSK) != NODE_TYPE_REQUEST) {// TODO: verify the removed
                                                                                        // section is a request type of
                                                                                        // child and the request id
                                                                                        // matches this
            ...
            if (rem != 0) {
                ...
                if (remaining == 0) {
                    // This state should only ever occur when a node is inflight, or... if an inner
                    // node has existance mask of 0... sigh
                    ...
                    this.nodeData.setAllChildrenAreLeaf(nodeId, false);// Children dont exist, therefor set them to
                                                                       // false
            ...
            if (this.nodeData.isNodeRequestInFlight(nodeId)) {// Also only need to do this after/if there are removals
                                                              // to be done
                int requestId = this.nodeData.getNodeRequest(nodeId);
                var request = this.childRequests.get(requestId);// TODO: do not assume request is childRequest (it will
                                                                // probably always be)
        ...
        if (childExistence == 0) {
            // We need to change the node from inner to leaf as it does not have any
            // children
            ...
            if (this.nodeData.getNodeGeometry(nodeId) == NULL_GEOMETRY_ID) {
                ...
                if (!this.nodeData.isNodeGeometryInFlight(nodeId)) {
                    if ((this.watcher.get(pos) & UPDATE_TYPE_BLOCK_BIT) != 0) {
                        throw new IllegalStateException(
                                "Watcher was already watching for geometry update, but geometry was null");
                    }
                ...
                // TODO: figure out a better way to mark this for tracing verificaction and like
                // less confusion
                // (instead of like EMPTY_GEOMETRY_ID do like INFLIGHT_GEOMETRY_ID)
                this.nodeData.setNodeGeometry(nodeId, EMPTY_GEOMETRY_ID);
            }

            if (this.nodeData.getChildPtr(nodeId) != SENTINEL_EMPTY_CHILD_PTR) {// This should only ever be the sentinal
                                                                                // ptr
                throw new IllegalStateException();
            }
    ...
    private void _removeRequest(int reqId, NodeChildRequest req, long pos) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if ((cId & NODE_TYPE_MSK) != NODE_TYPE_REQUEST || (cId & REQUEST_TYPE_MSK) != REQUEST_TYPE_CHILD
                    || (cId & NODE_ID_MSK) != reqId) {
    ...
    private void _recurseRemoveNode(long pos, boolean onlyRemoveChildren) {
        ...
        if (type == NODE_TYPE_INNER || type == NODE_TYPE_LEAF) {
            ...
            // Need to recurse into childExistence that exist, this is xor between a request
            // mask if there is and the
            // childRequest
    ...
    private void finishRequest(SingleNodeRequest request) {
        ...
        this.activeSectionMap.put(request.getPosition(), id | NODE_TYPE_LEAF);// Assume that the result of any single
                                                                              // request type is a leaf node
    ...
    private void finishRequest(int requestId, NodeChildRequest request) {
        ...
        if (parentNodeId == -1 || (parentNodeId & NODE_TYPE_MSK) == NODE_TYPE_REQUEST) {
            throw new IllegalStateException(
                    "CRITICAL BAD STATE!!! finishRequest tried to finish for a node that no longer exists in the map or has become a request type somehow?!!?!!"
                            + WorldEngine.pprintPos(request.getPosition()) + " " + parentNodeId);
        }
        ...
        if (parentNodeType == NODE_TYPE_LEAF) {
            ...
            for (int childIdx = 0; childIdx < 8; childIdx++) {
                ...
                if (childExistence == 0) {
                    // This is an ok error if it happens the request with a child state should never
                    // be zero
                ...
                if ((pid & NODE_TYPE_MSK) != NODE_TYPE_REQUEST) {
                    throw new IllegalStateException("Put node in map from request but type was not request: " + pid
            ...
            if ((this.activeSectionMap.put(request.getPosition(), NODE_TYPE_INNER | parentNodeId)
                    & NODE_TYPE_MSK) != NODE_TYPE_LEAF) {
                throw new IllegalStateException();
            }
            this.invalidateNode(parentNodeId);
            ...
            // For this, only need to add the nodes to the existing child set thing (shuffle
            // around whatever) dont ever have to remove nodes
            ...
            if (oldChildPtr != SENTINEL_EMPTY_CHILD_PTR) {
                // Ok so technically, it _is ok_ to just add to the end of the childPtr,
                // however, imo that is stupid
                // and it should follow the logical allocation with respect to the 8 child
                // indices
                // this means, need to extract the child indices already in the ptr (or
                // technically could use the child existance? but having both and doing
    ...
    public void processRequest(long pos) {
        ...
        if (nodeId == -1) {
            ...
            // Logger.warn("Got request for pos " + WorldEngine.pprintPos(pos) + " but it
            // was not in active map, ignoring!");
            return;
        }
        int nodeType = nodeId & NODE_TYPE_MSK;
        nodeId &= NODE_ID_MSK;
        if (nodeType == NODE_TYPE_REQUEST) {
            Logger.error("Tried processing request for pos: " + WorldEngine.pprintPos(pos)
                    + " but its type was a request, ignoring!");
            return;
        ...
        if (WorldEngine.getLevel(pos) == 0) {
            Logger.error("Requests cannot exist for bottom level nodes. at: " + WorldEngine.pprintPos(pos)
                    + ". Ignoring request");
            return;
        }

        // TODO: ADJUST AND FIX THIS TO MAKE IT REMOVE THE LAST THING IN QUEUE OR
        // SOMETHING
        // if (this.activeNodeRequestCount > 100 && WorldEngine.getLevel(pos) < 2) {
        // Logger.info("Many active requests, declining request at " +
        ...
        // TODO:
        // Make it so that if a request is not in flight it has an invalid/null request
        // entry

        // NOTE: inner nodes /w request should check they have geometry independenently
        // of being inflight

        // TODO: FIXTHIS:
        // https://discord.com/channels/973046939375505408/973046939375505411/1328785093812031489
        // this causes things to go bad, when racing the gpu, i.e. this becomes an inner
        // node that has geometry and there is now a request for it
        // in this case we should not mark the node as inflight as it casuse very bad
        // things to happen
        ...
        if (nodeType == NODE_TYPE_LEAF) {
            ...
            if (this.nodeData.getNodeGeometry(nodeId) == NULL_GEOMETRY_ID) {
                ...
                Logger.warn("Got request for leaf that doesnt have geometry, this should not be possible at pos "
                        + WorldEngine.pprintPos(pos));
                if (!this.watcher.watch(pos, WorldEngine.UPDATE_TYPE_BLOCK_BIT)) {
                    Logger.warn("Node: " + nodeId + " at pos: " + WorldEngine.pprintPos(pos)
                            + " got update request, but geometry was already being watched");
                }
                return;
            }
    ...
    private void makeLeafChildRequest(int nodeId) {
        ...
        if (childExistence == 0) {
            if (!this.topLevelNodes.contains(pos)) {// Top level nodes are special, as they can have a request with
                                                    // child existence of 0 for performance reasons
                // Rate limit warning to reduce log spam - only log once every 5 seconds
                long currentTime = System.currentTimeMillis();
                if (currentTime - lastExistenceMaskWarningTime > 5000) {
                    if (suppressedExistenceMaskWarnings > 0) {
                        Logger.warn("Not creating a leaf request with existence mask of 0 (suppressed " +
                                suppressedExistenceMaskWarnings + " similar warnings) at pos",
                                WorldEngine.pprintPos(pos));
                        suppressedExistenceMaskWarnings = 0;
                    } else {
                        Logger.warn("Not creating a leaf request with existence mask of 0 at pos",
                                WorldEngine.pprintPos(pos));
                    }
                    lastExistenceMaskWarningTime = currentTime;
                } else {
                    suppressedExistenceMaskWarnings++;
                }
                this.nodeData.unmarkRequestInFlight(nodeId);
                this.invalidateNode(nodeId);
                return;
        ...
        // Only request against the childExistence mask, since the guarantee is that if
        // childExistence bit is not set then that child is guaranteed to be empty
        for (int i = 0; i < 8; i++) {
            if ((childExistence & (1 << i)) == 0) {
            ...
            if (pid != -1) {
                ...
                if ((pid & NODE_TYPE_MSK) == NODE_TYPE_LEAF) {
                    extra = " type leaf: pos " + WorldEngine.pprintPos(this.nodeData.nodePosition(pid))
                            + " hasRequest: " + this.nodeData.isNodeRequestInFlight(pid);
                }
                throw new IllegalStateException(
                        "Leaf request creation failed to insert child into map as a mapping already existed for the node! pos: "
                                + WorldEngine.pprintPos(childPos) + " id: " + pid + " for parent "
                                + WorldEngine.pprintPos(pos) + " extra " + extra);
            }
    ...
    private void processInnerRequest(long pos, int nodeId) {
        ...
        if (VERIFY_NODE_MANAGER_OPERATIONS) {
            ...
            if (geo != NULL_GEOMETRY_ID && inflight) {
                // Having a EMPTY_GEOMETRY_ID and inflight is valid unfortunatly due to
                // conditions when making an
                // inner node into a leaf node when child existance is set to zero and it has no
                // geometry
        ...
        if (!this.nodeData.isNodeGeometryInFlight(nodeId)) {
            if (!this.watcher.watch(pos, WorldEngine.UPDATE_TYPE_BLOCK_BIT)) {
                // Logger.info("Node: " + nodeId + " at pos: " + WorldEngine.pprintPos(pos) + "
                // got update request, but geometry was already being watched");
    ...
    // TODO: Think plan for this is to add new flag to NodeStore to indicate if
    // geometry mesh request is inflight
    // this used for state verification and not emitting/assuming things
    // e.g. current issue is if an inner node wants/needs to convert into a leaf
    // node, but the inner node has no geometry
    // how to deal with that?? e.g. inner node geometry gets cleared but then the
    // childExistance gets set to 0
    // it needs to become a leaf node

    public void removeNodeGeometry(long pos) {
        int nodeId = this.activeSectionMap.get(pos);
        if (nodeId == -1) {
            // Logger.warn("Got geometry removal for pos " + WorldEngine.pprintPos(pos) + "
            // but it was not in active map, ignoring!");
            return;
        }
        int nodeType = nodeId & NODE_TYPE_MSK;
        ...
        if (nodeType == NODE_TYPE_REQUEST) {
            ...
            // Logger.warn("Tried removing geometry for pos: " + WorldEngine.pprintPos(pos)
            // + " but its type was a request, ignoring!");
            return;
        }
        // this.clearId(nodeId);
        ...
        if (nodeType == NODE_TYPE_INNER) {
            ...
            // TODO? think about maybe only doing it if all children of the parent are leaf
            // nodes aswell

            if (this.topLevelNodes.contains(pos)) {
                // We are asked to remove the geometry of a top level leaf node, which we cannot
                // do
                int geo = this.nodeData.getNodeGeometry(nodeId);
                if (geo == NULL_GEOMETRY_ID || geo == EMPTY_GEOMETRY_ID) {
                    ...
                    Logger.warn(
                            "Tried removing geometry from top level node which is not allowed, disregarding request");
                    // TODO: probably do
                    // this.clearId(nodeId);
                    return;
                }
    ...
    private void clearGeometryInternal(long pos, int nodeId) {
        ...
        // TODO: if isNodeGeometryInFlight is true and geometryId == NULL_GEOMETRY_ID,
        // probably need to
        ...
        if (meshId != NULL_GEOMETRY_ID && meshId != EMPTY_GEOMETRY_ID) {
            ...
            if (this.watcher.unwatch(pos, WorldEngine.UPDATE_TYPE_BLOCK_BIT)) {
                throw new IllegalStateException("Unwatching position for geometry removal at: "
                        + WorldEngine.pprintPos(pos) + " resulted in full removal");
            }
            ...
            if (meshId == NULL_GEOMETRY_ID) {
                // Logger.info("Tried removing geometry of internal node but geometry was
                // null");
            }
        }
    }
    ...
    public boolean writeChanges(GlBuffer nodeBuffer) {
        ...
        this.nodeUpdates
                .forEach((int i) -> this.nodeData.writeNode(UploadStream.INSTANCE.upload(nodeBuffer, i * 16L, 16L), i));
    ...
    public void addDebug(List<String> debug) {
        debug.add("NC/IF: " + this.activeSectionMap.size() + "/"
    ...
    // TODO: need to figure out what happens if an inner node gets marked with child
    // existence of 0
    // it should become a leaf node
    // however, if the node doesnt have geometry attached that would put it in an
    // invalid state so need to figure out
    // a solution for this

    private int verifyRequest(long pos, int node, int cActiveExistence, LongOpenHashSet seenPositions,
        ...
        if (type == NODE_TYPE_REQUEST) {
            ...
            if (this.nodeData.isNodeRequestInFlight(node)) {
                ...
                if (req.isSatisfied()) {// If a request is satisfied it should not be in the array
                    // The exception to this rule is the top level nodes, and only if they are leaf
                    // nodes
...

            {
                ...
                if (hasGeometry && awaitingGeo) {
                    // We assume if the geometry is EMPTY, that what happened was an inner node just
                    // got convertex into a leaf node and is now awaiting its geometry
                    // if (type != NODE_TYPE_LEAF || this.nodeData.getNodeGeometry(node) !=
                    // EMPTY_GEOMETRY_ID)
                    // throw new IllegalStateException();
                    // HOWEVER, what can happen is that before we recieve the geometry for the node,
                    // thus clearing the geometryInFlight
                    // is that we get a request and childexistance change and all the children
                    // recieved,
